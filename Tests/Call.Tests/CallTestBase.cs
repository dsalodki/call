...
using Call.Migrations.SeedData;
...
namespace Call.Tests
{
    public abstract class CallTestBase : AbpIntegratedTestBase<CallTestModule>
    {
        ...
        protected void UsingDbContext(Action<CallDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<CallDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<CallDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<CallDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CallDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CallDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<CallDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CallDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<CallDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<CallDbContext, Task<T>> func)
        {
            T result;
